...
- The pipeline runs end-to-end for the 10 recordings already included in
  `out/`, using a corrected free-top cantilever building model (see
  `specs/01-physics-model.md` for the full before/after — this replaced an
  earlier "fixed at both ends" model that wasn't how real buildings are
  actually supported).
...
- `out/`'s precomputed results still use a fixed 7-floor building (see
  ...
  isn't supported yet — see `specs/03-live-archetypes.md`.
- No automated tests as a project convention, though
  `scripts/verify_physics.py` checks the building model's math against an
  independently-derived closed-form solution.
- There's an active, spec-driven development plan in `specs/` (with
  matching checks in `verification/`) — check there before assuming
  something described as "not done yet" here is still accurate.
- Two bugs reported directly against the Building Parameters panel (a
  parameter tweak was silently changing the Amplify zoom, and increasing
  story count cut the building off on screen) are fixed and verified — see
  `verification/04-slider-camera-fixes.md` for the root causes (including
  two follow-on issues, a fog-range mismatch and an OrbitControls damping
  drift, that the initial fix attempt surfaced) and how each was checked in
  a real browser.
- A systematic pass over every Building Parameter slider (see
  `verification/05-slider-fuzz-matrix.md`) found and fixed two more real
  bugs: **Stories = 1 rendered as a solid black box** (a `0/0` divide in
  the floor color gradient), and **switching the Earthquake Record while
  any Building Parameter was off its default silently reloaded the static
  7-story data**, leaving the panel's sliders showing values that no
  longer matched the rendered building. The same pass fuzzed the physics
  endpoint itself across every parameter's min/max/combined extremes and
  found nothing broken there.
