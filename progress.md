...
## Spec 16 — cinematic layer (goal loop)

### Task 1: isolated checkout and baseline (2026-09-23)

- Created `.worktrees/goal-16-cinematic-layer` on `goal/16-cinematic-layer` from clean `main` at `6dcaa26`; bridged the six ignored project directories with junctions and copied `AGENTS.md`. No product files changed and no commit was made.
- `graphify update .` passed inside the worktree (424 nodes, 614 edges); the wrapper needs sandbox escalation to read its installed package.
- Baseline `git status --short --branch`: `## goal/16-cinematic-layer`, clean.
- All 13 baseline Node scripts passed: `check_index_syntax`, `fft_check`, `check_time_domain` (7176 comparisons), `check_footprint_area`, `check_sway_gain`, `check_furniture_gain`, `check_column_shape`, `check_hysteresis_panel`, `check_floor_rotation`, `check_story_heights`, `check_damage_render`, `check_collapse_readout`, and `check_collapse_kinematics` (including deterministic keyframe hashes). `fft_check.mjs` generated its JS fixture; the SciPy comparison remains for the final suite.
- The earlier post-merge `check_column_shape.mjs` exception did not recur: it passed all comparisons against this worktree's `main` baseline.
- Revalidation rulings were completed during the approved plan stage: preserve disabled auto-orbit; use the live scaled acceleration tail; keep the existing apply/render/revert camera shake lifecycle; add JavaScript handling for reduced motion.

### Task 2: filtered acceleration camera shake (commit 5b8ecaa)

- Added a separate `CINEMATIC-HELPERS` block with an exact digital 8 Hz one-pole low-pass, per-load filtered X/Y trace, whole-trace gain cap, and interpolation at `animTime`. Replaced the wall-clock sine with an X/Z camera-only render offset that is immediately reverted.
- `check_cinematic.mjs` was first run against the missing sentinel and failed as expected; then passed its 20-frequency response, exact corner, 1000-sample independent recurrence, 100× waveform-shape, time determinism, and reduced-motion helper checks. `check_index_syntax`, sway/furniture-gain, and `check_time_domain` (7176 comparisons) passed. `git diff --check` passed.
- Ruling: the ground-acceleration cache is rebuilt on every `applyLoadedData()` call from the loaded signal and `time` sample interval. It is not reused across live recomputes, so synthetic scaling cannot leave stale shake data.
- `graphify update .` ran after commit and found no topology changes. The check script is in the gitignored shared `claude_scripts/` directory, per repository convention; the product code was committed.

### Task 3: failing-story camera and manual override (commit 99fc678)

- `check_cinematic.mjs` first failed because `firstCinematicOnset` was absent. Added earliest finite onset selection across axes with the 0-based index converted to story number and 1.5 s lead. No onset produces no move.
- Extended `frameCameraToFloor()` with an optional target centre and cinematic duration; it still leaves `maxDistance`, `camera.far` and fog alone and calls `tweenCameraTo()`, which retains `scheduleAutoOrbit()`. After computed detachment, follow the matching computed body's sampled centre of mass using a critically damped 0.4 s update. No Rapier step is added to playback.
- Manual OrbitControls start, Floor selection, and Back to full view cancel the cinematic move/follow. A View affordance re-arms it; if the lead time has passed, it waits for a later playthrough rather than pulling the camera back. The pre-existing disabled auto-orbit preference is intact.
- `check_cinematic.mjs`, `check_index_syntax.mjs`, and `git diff --check` passed. The camera checks cover onset selection, no-onset, damping direction, and the per-floor framing invariants. Real-browser behavior remains for Task 5.
- `graphify update .` ran after commit and found no topology changes.

### Task 4: slow-motion clock and replay (commit 831ccda)

- `check_cinematic.mjs` first failed because `slowMotionRate` was absent. Added the spec's cubic 0.8 s ramp to 0.15, hold to 3 s after onset, and 1.2 s exit, with a pure rate function and clamped replay start.
- `animTime` is advanced exactly once by `delta * speed * rate(animTime)`; the seek slider remains a direct record-time mapping. The playback path still samples spec 15's keyframe buffer and never steps Rapier. The replay button moves the playhead and uses `togglePlayback()` only if paused, with no second timer or animation loop.
- View now has persistent checkbox states for camera shake, cinematic camera, and slow motion. Slow motion and replay appear only when a computed onset exists. Replaying resets camera playthrough state but preserves the user's toggles. Automatic record wrap also starts a new playthrough.
- `check_cinematic.mjs` passed independent cubic endpoints, 50 ramp points, 1000-step rate integration, reduced-motion rate 1, clamped replay and seek-source checks. `check_time_domain.mjs` passed 7176 comparisons, `check_index_syntax.mjs` parsed, `check_collapse_kinematics.mjs` passed, and `git diff --check` passed. Real-browser behavior remains for Task 5.
- `graphify update .` ran after commit and found no topology changes.

### Task 5: reduced motion and real-browser behavior (commit 82faa49)

- Added a live `prefers-reduced-motion` listener. Shake and slow-motion controls become disabled while the preference is active without losing their saved checked values; shake samples are exactly zero, the rate is 1, and the cinematic camera tween duration is 1 ms in code. The Signals pen is untouched. Browser media emulation was unavailable through the connected browser controls, so the live reduced-motion interaction remains an open verification check rather than a claimed pass.
- The real app ran from this worktree's Flask server at `127.0.0.1:8001` (port 8000 was already serving a different 404 app). In the 375×812 browser viewport, the four-story KOCAELI_AYD demo converged in 3 iterations: first X/Y onset story 3 at 35.50/35.82 s, computed detachment story 3 at 80.54 s, and animated cascades stories 2/1 at 81.92/82.43 s. The camera framed the first onset, the Floor selector now displayed Floor 3, Replay returned to about 34.0 s, manual orbit exposed re-enable, and Back to full view returned the selector to Full building. The falling block, dust, and fog remained visible at a desktop zoom-out near 82.9 s. Both camera toggles and the slow-motion toggle worked in both directions; shake/camera state survived a record switch, and the hidden slow-motion control retained its unchecked state on an elastic record switch.
- The 375×812 mobile controls had no horizontal overflow (`controls.scrollWidth === controls.clientWidth === 364`, `body.scrollWidth === 375`); Slow motion and Replay were visible only with onset. Time, Frequency and Hysteresis all opened with live data, and browser error/warning logs were empty. The verification text says "four Signals tabs," but the merged viewer has three; correct that wording during Task 6 documentation. The three fixed interior lights and `updateFogRange()` path were unchanged.
...
## Tasks 2–5 — Measured signals, convergence, experiments (2026-09-23)

- The Frequency view now compares measured elastic and nonlinear output spectra from relative floor displacement, with each peak level and frequency printed. The corrected identity uses the solver's saved elastic base and pseudo-force on the solve grid, then transforms the aligned physical-time histories onto a common display grid. The solver uses separate FFT padding for the elastic base and feedback correction; the spec's literal single-solver-grid criterion is therefore inapplicable. The yielding PARK2004_HOG X case gave complex closure 1.970e-16 and elastic discrepancy 1.000. Record this revalidation ruling and correct the spec/verification wording in Task 6.
- The fourth drawer tab plots the actual `residual_history` on a logarithmic scale with the tolerance rule and iteration status. Forced non-convergence retains the history and produces no collapse event or identity panel. An inline SVG explains the nonlinear feedback loop. The initially disconnected line reported by the user was joined and visually rechecked in the browser.
- The on-demand superposition experiment uses an optional float64 relative-floor trace appended to `/compute`, keeping the normal float32 animation payload intact. An elastic ANZA1_CIDLA test gave error 0.000e+00; the yielding PARK2004_HOG X 2×/4× comparison gave 0.823 (>0.1). The interface makes exactly two requests on a new test and caches both. The parameter sweep makes sequential client-driven requests at 20 log-spaced levels for each elastic/nonlinear series; it plots partial results, cancels, resumes, and invalidates the cache on parameter changes. The ANZA1_CIDLA control gave a through-origin residual of 8.205e-17. It had no collapse event, and the plot did not invent one. Markers use only returned yield/collapse data. No `/sweep` endpoint was added.
- Focused verification passed: `check_signal_trace.py`, `check_signal_torsion.py`, `check_signal_sweep.py`, `check_signal_nonconvergent.py`, `check_signals_panels.mjs`, `check_time_domain.mjs` (7176 comparisons), `fft_check.mjs` plus `fft_check_scipy.py` (complex FFT relative errors 3.383e-14 and 3.397e-14), `check_hftd_elastic_baseline.py` (140 bit-identical cases), `check_ground_accel_block.py` (all checks, including zero trailing bytes), and `check_damage_blocks.py` (all checks, including 64 subsets). The inline module syntax, Python compile, and `git diff --check` passed. In a preliminary real-browser pass, the four tabs, identity, superposition, full 40-request sweep, and diagram rendered with no console errors; full Task 6 browser/regression/documentation pass remains for the separate session.
- A final 20-level nonlinear PARK2004_HOG X sweep passed: first sampled yield 1.605×, first sampled collapse event 2.200×. Levels through 14.591× converged; 20× did not and produced no invented yield/collapse marker. Peak drift rose from 0.01780 at 2.200× to 0.10092 at 14.591×, without the universal >10×/10% flatline assumed by the original provisional verification text. This is a measured record-dependent outcome for the Task 6 correction.

## Spec 17 — Task 6 revalidation and closeout (2026-09-23)

Corrected the provisional spec and paired verification criteria against the measured implementation. The elastic base and nonlinear correction use separate solver FFT padding, so the corrected identity is evaluated only after their physical-time histories are aligned on the common display grid. Yield, collapse, and nonconvergence are record-dependent; acceptance requires showing returned outcomes and never fabricating collapse or a universal flatline. A lower nonlinear output peak is a possible physical result, not a universal criterion. The implemented sweep is client-driven, cancellable `/compute` work; no `/sweep` endpoint ships.

Documentation updated: README, math PDF Part L (45 pages), `specs/README.md`, `specs/COURSE-CONCEPTS.md`, `knowledge/index_html.md`, `knowledge/server.md`, and both local AGENTS handoffs. Graphify refreshed to 438 nodes / 631 edges / 32 communities. `check-docs-drift` found README terms, Part L derivation, the verification scripts, current unmerged branch status, and the new symbols in the relevant knowledge notes.

Fresh 20-level `check_signal_yielding_sweep.py` pass: PARK2004_HOG X first yielded at 1.605×; collapse events returned from 2.200× through 14.591×; 20× did not converge and produced no fabricated peak/event. The underlying values include drift 0.0178016 at 2.200× and 0.100920 at 14.591×. Focused checks for trace identity, 3N torsion, elastic/nonlinear sweeps, forced nonconvergence, panels, FFT, baseline, ground acceleration, and damage blocks passed. The full applicable Python and JavaScript regression batch exited 0; `verify_torsion.py` checks 1–8, restart/handoff checks, and the spec 11 regression bundle all passed, with zero generated `out/` diff. `git diff --check` passed.

Real browser verification used the worktree Flask app on port 8001. Desktop 1440×900: all four tabs and SVG diagram were visible; PARK2004_HOG X at 2× converged in 3 iterations, with both measured output peaks at 0.62 Hz; the aligned corrected identity closure was 1.51e-16 and elastic discrepancy 1.16e-1 for that live run; superposition error was 8.64022061e-1. The 20× run showed “Did not converge,” final residual 2.949e-5, and no collapse inference. Cancelling the active sweep retained 31 samples; repeating the cached test sent no new request. At 375×812, the four-tab strip fit its 323px container, body/drawer had no horizontal overflow, the SVG scaled to 324×94, and the convergence canvas to 324×240. No application JavaScript errors occurred; the only console item was the missing `/favicon.ico` 404.

Task 6 complete on local branch `goal/17-signals-view-of-nonlinearity`; not merged or pushed. Backend mirror remains owed only after a user-approved merge because `server.py` changed for optional analysis request fields.

## Spec 18 — Code-design mode (2026-09-23, `goal/18-code-design-mode`)

- Based isolated branch on `main` at `4b8459d` after spec 17 landed. Main checkout's unrelated untracked files were preserved. No merge, push, or backend mirror.
- Source pass: HBRI lists final BNBC 2020 Part 2, but its final PDF was inaccessible; the accessible IISEE chapter is page-labelled 2017. `verification/18-code-design-mode.md` records status by constant. `specs/18-code-design-mode.md` records corrected interfaces, assembled-frame sizing, source limits, mass consistency, record-dependent ranking, and Part M (Part L belongs to 17).
- Added pure `code_design.py`: illustrative seismic weight and base shear, force distribution, X/Y assembled-frame `k0_profile` and static drift checks, reinforcement-first strength sizing, axial and P-Delta checks, bounded nonconvergence. The solver equations in `mdof_response.py` were untouched. `/design` returns output profiles and audit; `/compute` accepts scalar `rho_longitudinal` and echoes it as additive header metadata, without changing binary block order.
- Added explicit preview/apply Design Mode, preserved Manual controls, visible scope statement, generated profile readout, single-scene cached A/B comparison, measured drift/ductility charts, elastic controls, and paired optional IDA sweep. No backend mirror yet; it is user-invoked after approved merge.
- Focused verification: `claude_scripts/verify_code_design.py` passes six independent demand cases, N=1/7/20 X/Y static drift, strength, axial, theta, 3N/manual bit identity, infeasible-design reporting, endpoint validation, and one real `/compute` response. For the original N=7 geometry the `12EI/h³` shortcut differs from actual `k0` by 317.9%.
- Node regression: 14/14 checks pass, including inline-module syntax. `check_column_shape.mjs` is a one-time pre-spec-14 baseline script tied to `main` and was excluded per the existing handoff.
- Real browser: Code preview did not alter the loaded building; Apply loaded the generated member profile. N=7 generated column X depths taper 0.341→0.239 m. Switching back to Manual restored 7 stories and 1,000,000 kg floor mass. N=20 preview flagged assembled period 4.43 s above the draft 2 s equivalent-static limit. Desktop N=20 single-scene measured ~32.6 fps and mobile 375×812 ~34.7 fps; mobile document width stayed 375 px. Temporary FPS instrumentation was removed. Browser console errors: 0.
- Record-dependent A/B: ANZA1_CIDLA at 1× produced no collapse in either building; first-story peak drift ~0.01% for both; elastic fundamental periods 1.89 s (code) vs 3.07 s (arbitrary). At 20× neither collapsed; first-story drift 0.28% arbitrary vs 0.13% code. The paired 20-point sweeps found no sampled collapse in either curve through 20×. No collapse separation is claimed.
- Part M of the 46-page math PDF now includes the full scope statement, demand chain, assembled-frame inversion, capacity caveat, and source-status table. README, knowledge notes, AGENTS.md, spec index, and course concepts were updated. No new dependency was added.

## Spec 19 — UI workspace (goal/19-ui-workspace)

### Tasks 1–6 (2026-09-24)
- Baseline at main 1f643ff (clean tree; the spec's "dirty" files were committed as 1f643ff). 15/15 Node checks passed before edits.
- Request parity (V3): 19 POST bodies captured from the old UI via the same element-id script (claude_scripts/spec19_request_parity.js) and replayed on the new UI: all 19 byte-identical (manual sliders, soft/weak story, magnitude/distance/depth, nonlinear w/ intensity+torsion, /design, code Apply, return to Manual).
- New layout: header (Setup toggle, record select + info, Signals toggle) / Setup | scene | Signals grid / persistent transport / phone bottom nav. Hiding animates one grid track; closed panels are inert + visibility/opacity hidden.
- Text walls replaced: post-failure notice 61 px / 12 words at 1440x900 (was ~175 words); result card <=4 metrics; design preview card + Details table; convergence as three fields. Full values in one Result details dialog (real KOCAELI_AYD demo: 3 iterations, onsets story 3 @ 35.50 s X / 35.82 s Y, detachment story 3 @ 80.54 s, X residual 2.535e-5 — same as the spec audit).
...
### Follow-up (2026-09-24): cinematic sequence, fall follow, overlays
- Cinematic camera is now a shot list (`cinematicShots()` in CINEMATIC-HELPERS): close-up at the first computed onset (lead 1.5 s), hold 4 s, zoom out to the full building, close-up again at the first detachment. A wide shot shorter than 2 s is dropped; an onset inside the failure lead folds into it. Slow motion and Replay stay anchored on the detachment. Scrubbing never fires a past shot (`nextShotIndex`).
- Fall follow tracks `collapseFallTarget()`: mass-weighted centre of active computed/cascade bodies, and of rubble once a floor shatters. The old follow tracked one slab whose buffer track stops at its fracture, so the camera stayed at story height. Demo run: target height 10.6 at 80.3 s → 1.27 at 84 s → 0.75 at 90 s (ground).
- Post-failure notice and illustrative chip moved to the top-left of the scene (they covered the base of the fall); scene status pill removed at the user's request (kept as screen-reader text); elastic ghost label shortened (×scale / off / n/a, full text in its title).
- Checks: check_cinematic.mjs extended (shot list, crowding, scrub/replay indexing, follow wiring); spec19_media/check_cinematic_sequence.mjs (real demo) all pass; all 8 clips re-captured, check_help_media.mjs passes; 17/17 Node checks.
- Follow-up 2: scrubbing after a failure back to an earlier time now re-frames on scrub end (`resyncCinematicCamera`): the viewer's own view before the first shot / during the wide shot / after the hand-back, the onset close-up inside it; a scrub into the failure undoes a stale close-up. The viewer's camera is saved when a shot first takes over (`cinematicSavedView`) and restored by the wide shot and by a new `return` shot 6 s (record time) after the detachment, which also ends the fall follow; loop wrap restores too. Manual orbit / Camera focus / Back to full view discard the saved view. Real demo (check_cinematic_sequence.mjs): hand-back pose equals the pre-cinematic pose exactly; scrubs to 50 s, 36 s and 10 s give own view, floor-3 close-up, own view.

## Spec 20 — Structural visual polish (codex/spec20-structural-polish)

### Prototype and review (2026-09-24/25)
- Isolated worktree `.worktrees/codex-spec20-structural-polish` from `0c12ed7`. Baseline Flask app from the untouched main checkout on 127.0.0.1:8010, prototype on 8011 (8000–8002 were held by unidentified servers, left alone).
- Matched captures (claude_scripts/spec20_media/shoot.mjs, collapse.mjs, charts.mjs, loader.mjs): same viewport, record, paused time and app-framed camera; the served page is intercepted only to expose the camera for posing, identically on both servers.
- Changes: concrete materials (metalness 0, higher roughness), restrained floor gradient, retuned key/fill/rim/ambient/hemisphere, brighter navy background with matching fog, later bloom threshold, shadow box fitted per rebuild (`fitShadowBounds`, SHADOWFIT-HELPERS; normal bias from texel size), 400-unit ground, 50×50 grid retuned, slate foundation. User-requested additions: spec 4 stacked-floors loader restored over the scene only; elastic label "Recomputing" (was always "Updating building…"); Time/Frequency titles moved above the plots, end ticks aligned inward.
...
- Collapse demo cache: the demo's `/compute` response is saved in `out/collapse_demo/` (`request.json` + `response.bin`, 5.1 MB; two solves were byte-identical). A Run collapse analysis request identical to `request.json` loads the file instead of solving: click → Ready 77.7 s → 0.24 s, fall animation ready in ~5.6 s, same 0/0/0/0/2/3/4 timeline, 0 errors. Any other request still calls `/compute` (checked with one intensity step lower). Regenerate with the curl command in the README after solver changes.
- Collapse animation loader: the Rapier buffer was built in 200-step chunks of ~600 ms, so the loader repainted about once every 0.6 s. It now steps within a ~30 ms budget per chunk (the buffer is chunk-size independent, check 3) and the stacked-floor keyframes animate only transform/opacity, so they stay on the compositor. Main-thread frame gap during the build: median 609 ms → 35 ms; build 4.7 s → ~5.7–6.9 s.
- Signals panes: at panel heights under ~750 px the Convergence metrics and the Hysteresis/Convergence "Go to Analysis" empty-state button were cut off. The chart now takes the space the notes leave (min 120 px), the empty-state note is centred over the pane, and only the active pane may scroll. Checked at 1440×900, 1280×720, 1024×768 and 390×844.
